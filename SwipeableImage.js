...
export default function SwipeableImage({ user, willLike, willPass }) {
  return (
    <View>
      <Image source={{ uri: user.link }} style={styles.photo} />
      {willLike && (
      ...
      <View style={styles.textContainer}>
        <View style={styles.textRow}>
          ...
          <Text style={[styles.textSecondary, styles.textShadow]}>{18}</Text>
        </View>
        <View style={styles.textRow}>
          <FontAwesome name="map-marker" size={20} color="white"></FontAwesome>
          <Text style={[styles.textSecondary, styles.textShadow]}>{"Москва"}</Text>
        </View>
      </View>
    </View>
  )
